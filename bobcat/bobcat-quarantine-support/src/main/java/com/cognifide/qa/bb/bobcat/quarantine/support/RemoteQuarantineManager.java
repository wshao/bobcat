...
public class RemoteQuarantineManager implements QuarantineManager {
  ...
  @Override
  public void putElementToQuarantine(String url, QuarantinedElement element) throws TestQuarantineEception {
    ...
    try (CloseableHttpResponse response = httpClient.execute(httpPost)) {
      if (response.getStatusLine().getStatusCode() >= 400) {
        ...
        throw new TestQuarantineEception("Not able to add a test to quarantine: " + resposneMessage);
      }
    } catch (IOException ex) {
  ...
  @Override
  public void removeElementFromQuarantine(String url, QuarantinedElement element) throws
    TestQuarantineEception {

    HttpDelete httpDelete = new HttpDelete(url);
    try {
      URI uri = new URIBuilder(httpDelete.getURI())
        .setParameter("testClass", element.getTestClass())
        .setParameter("testMethod", element.getTestMethod())
        .build();
